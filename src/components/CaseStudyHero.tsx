...
export default function CaseStudyHero({ project }: { project: ProjectCaseStudy }) {
  ...
  return (
    <section ref={containerRef} className="relative w-full h-auto min-h-[75vh] lg:min-h-0 lg:h-[75vh] lg:h-[80vh] flex flex-col justify-center lg:justify-end p-0 lg:p-16 overflow-hidden bg-[#0c0c0d]">
      
      {/* Back Button */}
      <Link href="/#work" className="absolute top-6 left-6 lg:top-12 lg:left-12 group flex items-center gap-3 z-50">
        <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-full border border-white/20 flex items-center justify-center bg-black/50 backdrop-blur-md group-hover:bg-white group-hover:text-black transition-all duration-300">
        ...
        <span className="font-mono text-xs uppercase tracking-widest hidden lg:block opacity-0 group-hover:opacity-100 transition-opacity duration-300">Back</span>
      </Link>

      {/* =======================
          MOBILE LAYOUT (Cinematic Float)
          ======================= */}
      <div className="lg:hidden absolute inset-0 w-full h-full z-0 overflow-hidden bg-[#0c0c0d]">
      ...
      <div className="lg:hidden relative z-20 flex flex-col items-center justify-center w-full h-full px-6 pt-24 pb-12">
      ...
      {/* Desktop Gradients */}
      <div className="hidden lg:block absolute inset-0 z-10 bg-gradient-to-t from-[#0c0c0d] via-[#0c0c0d]/80 to-transparent pointer-events-none" />
      <div className="hidden lg:block absolute bottom-0 left-0 w-full h-[40vh] bg-gradient-to-t from-[#0c0c0d] to-transparent backdrop-blur-[2px] z-10 pointer-events-none" style={{ maskImage: "linear-gradient(to top, black 20%, transparent)", WebkitMaskImage: "linear-gradient(to top, black 20%, transparent)" }} />
      
      {/* Desktop Text */}
      <div ref={textRef} className="hidden lg:flex relative z-20 w-full max-w-7xl mx-auto flex-col items-start px-0 py-0">
        ...
        <div className="overflow-hidden">
          <h1 className="hero-reveal-text text-[6rem] lg:text-[8rem] lg:text-[10rem] font-inter font-bold tracking-tighter leading-[1.1] lg:leading-none text-white">
            {project.title}
          </h1>
        </div>
        
        {/* Scroll Indicator */}
        <div className="absolute bottom-[-2rem] right-0 lg:right-8 flex flex-col items-center gap-4 opacity-50 hidden lg:flex">
